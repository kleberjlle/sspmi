...
CREATE TABLE IF NOT EXISTS `itapoaap_sspmi`.`secretaria` (
  ...
  UNIQUE INDEX `idsecretaria_UNIQUE` (`idsecretaria` ASC) VISIBLE)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `itapoaap_sspmi`.`departamento` (
  ...
  INDEX `fk_departamento_secretaria1_idx` (`secretaria_idsecretaria` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `itapoaap_sspmi`.`coordenacao` (
  ...
  UNIQUE INDEX `idcoordenacao_UNIQUE` (`idcoordenacao` ASC) VISIBLE,
  INDEX `fk_coordenacao_departamento1_idx` (`departamento_secretaria_idsecretaria` ASC) VISIBLE,
  CONSTRAINT `fk_coordenacao_departamento1`
    FOREIGN KEY (`departamento_secretaria_idsecretaria`)
    REFERENCES `itapoaap_sspmi`.`departamento` (`secretaria_idsecretaria`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `itapoaap_sspmi`.`setor` (
  ...
  INDEX `fk_setor_coordenacao1_idx` (`coordenacao_departamento_iddepartamento` ASC, `coordenacao_departamento_secretaria_idsecretaria` ASC) VISIBLE,
  CONSTRAINT `fk_setor_coordenacao1`
    FOREIGN KEY (`coordenacao_departamento_secretaria_idsecretaria`)
    REFERENCES `itapoaap_sspmi`.`coordenacao` (`departamento_secretaria_idsecretaria`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `itapoaap_sspmi`.`cargo` (
  ...
  UNIQUE INDEX `nomenclatura_UNIQUE` (`nomenclatura` ASC) VISIBLE)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `itapoaap_sspmi`.`permissao` (
  ...
  UNIQUE INDEX `idpermissao_UNIQUE` (`idpermissao` ASC) VISIBLE,
  UNIQUE INDEX `nomenclatura_UNIQUE` (`nomenclatura` ASC) VISIBLE,
  UNIQUE INDEX `nivel_UNIQUE` (`nivel` ASC) VISIBLE)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `itapoaap_sspmi`.`usuario` (
  ...
  INDEX `fk_usuario_coordenacao1_idx` (`coordenacao_idcoordenacao` ASC) VISIBLE,
  INDEX `fk_usuario_departamento1_idx` (`departamento_iddepartamento` ASC) VISIBLE,
  INDEX `fk_usuario_secretaria1_idx` (`secretaria_idsecretaria` ASC) VISIBLE,
  INDEX `fk_usuario_telefone1_idx` (`telefone_idtelefone` ASC) VISIBLE,
  INDEX `fk_usuario_cargo1_idx` (`cargo_idcargo` ASC) VISIBLE,
  INDEX `fk_usuario_email1_idx` (`email_idemail` ASC) VISIBLE,
  INDEX `fk_usuario_permissao1_idx` (`permissao_idpermissao` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `itapoaap_sspmi`.`telefone_has_secretaria` (
  ...
  INDEX `fk_telefone_has_secretaria_secretaria1_idx` (`secretaria_idsecretaria` ASC) VISIBLE,
  INDEX `fk_telefone_has_secretaria_telefone1_idx` (`telefone_idtelefone` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `itapoaap_sspmi`.`telefone_has_departamento` (
  ...
  INDEX `fk_telefone_has_departamento_departamento1_idx` (`departamento_iddepartamento` ASC) VISIBLE,
  INDEX `fk_telefone_has_departamento_telefone1_idx` (`telefone_idtelefone` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `itapoaap_sspmi`.`telefone_has_coordenacao` (
  ...
  INDEX `fk_telefone_has_coordenacao_coordenacao1_idx` (`coordenacao_idcoordenacao` ASC) VISIBLE,
  INDEX `fk_telefone_has_coordenacao_telefone1_idx` (`telefone_idtelefone` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `itapoaap_sspmi`.`telefone_has_setor` (
  ...
  INDEX `fk_telefone_has_setor_setor1_idx` (`setor_idsetor` ASC) VISIBLE,
  INDEX `fk_telefone_has_setor_telefone1_idx` (`telefone_idtelefone` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `itapoaap_sspmi`.`email_has_setor` (
  ...
  INDEX `fk_email_has_setor_setor1_idx` (`setor_idsetor` ASC) VISIBLE,
  INDEX `fk_email_has_setor_email1_idx` (`email_idemail` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `itapoaap_sspmi`.`email_has_coordenacao` (
  ...
  INDEX `fk_email_has_coordenacao_coordenacao1_idx` (`coordenacao_idcoordenacao` ASC) VISIBLE,
  INDEX `fk_email_has_coordenacao_email1_idx` (`email_idemail` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `itapoaap_sspmi`.`email_has_departamento` (
  ...
  INDEX `fk_email_has_departamento_departamento1_idx` (`departamento_iddepartamento` ASC) VISIBLE,
  INDEX `fk_email_has_departamento_email1_idx` (`email_idemail` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `itapoaap_sspmi`.`email_has_secretaria` (
  ...
  INDEX `fk_email_has_secretaria_secretaria1_idx` (`secretaria_idsecretaria` ASC) VISIBLE,
  INDEX `fk_email_has_secretaria_email1_idx` (`email_idemail` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `itapoaap_sspmi`.`prioridade` (
  ...
  UNIQUE INDEX `idprioridade_UNIQUE` (`idprioridade` ASC) VISIBLE,
  UNIQUE INDEX `nomenclatura_UNIQUE` (`nomenclatura` ASC) VISIBLE)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `itapoaap_sspmi`.`protocolo` (
  ...
  UNIQUE INDEX `idprotocolo_UNIQUE` (`idprotocolo` ASC) VISIBLE,
  INDEX `fk_protocolo_usuario1_idx` (`usuario_idusuario` ASC) VISIBLE,
  INDEX `fk_protocolo_prioridade1_idx` (`prioridade_idprioridade` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `itapoaap_sspmi`.`sistemaOperacional` (
  ...
  UNIQUE INDEX `idsistemmaOperacional_UNIQUE` (`idsistemaOperacional` ASC) VISIBLE,
  UNIQUE INDEX `nomenclatura_UNIQUE` (`nomenclatura` ASC) VISIBLE)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `itapoaap_sspmi`.`tensao` (
  ...
  UNIQUE INDEX `idtensao_UNIQUE` (`idtensao` ASC) VISIBLE,
  UNIQUE INDEX `nomenclatura_UNIQUE` (`nomenclatura` ASC) VISIBLE)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `itapoaap_sspmi`.`corrente` (
  ...
  UNIQUE INDEX `idcorrente_UNIQUE` (`idcorrente` ASC) VISIBLE,
  UNIQUE INDEX `nomenclatura_UNIQUE` (`nomenclatura` ASC) VISIBLE)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `itapoaap_sspmi`.`equipamento` (
  ...
  INDEX `fk_equipamento_categoria1_idx` (`categoria_idcategoria` ASC) VISIBLE,
  INDEX `fk_equipamento_sistemaOperacional1_idx` (`sistemaOperacional_idsistemmaOperacional` ASC) VISIBLE,
  INDEX `fk_equipamento_tensao1_idx` (`tensao_idtensao` ASC) VISIBLE,
  INDEX `fk_equipamento_corrente1_idx` (`corrente_idcorrente` ASC) VISIBLE,
  INDEX `fk_equipamento_ambiente1_idx` (`ambiente_idambiente` ASC) VISIBLE,
  INDEX `fk_equipamento_local1_idx` (`local_idlocal` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `itapoaap_sspmi`.`etapa` (
  ...
  UNIQUE INDEX `idetapa_UNIQUE` (`idetapa` ASC) VISIBLE,
  INDEX `fk_etapa_equipamento1_idx` (`equipamento_idequipamento` ASC) VISIBLE,
  INDEX `fk_etapa_protocolo1_idx` (`protocolo_idprotocolo` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `itapoaap_sspmi`.`categoria_has_marca` (
  ...
  INDEX `fk_categoria_has_marca_categoria1_idx` (`categoria_idcategoria` ASC) VISIBLE,
  INDEX `fk_categoria_has_marca_marca1_idx` (`marca_idmarca` ASC) VISIBLE,
...
CREATE USER 'itapoaap_admin' IDENTIFIED BY 'pr3f31tur4@2024';
